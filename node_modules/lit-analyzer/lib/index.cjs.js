...
function mergeHtmlTags(tags) {
    ...
    try {
        for (var tags_1 = __values(tags), tags_1_1 = tags_1.next(); !tags_1_1.done; tags_1_1 = tags_1.next()) {
            ...
            if (existingTag != null) {
                mergedTags.set(tag.tagName, __assign(__assign({}, tag), { builtIn: tag.builtIn || existingTag.builtIn, global: tag.global || existingTag.global, declaration: tag.declaration || existingTag.declaration, description: tag.description || existingTag.description, attributes: mergeHtmlAttrs(__spread(tag.attributes, existingTag.attributes)), properties: mergeHtmlProps(__spread(tag.properties, existingTag.properties)), events: mergeHtmlEvents(__spread(tag.events, existingTag.events)), slots: mergeHtmlSlots(__spread(tag.slots, existingTag.slots)) }));
            }
            else {
...
function completionsAtOffset(document, offset, request) {
    ...
    if (intersectingAttr != null) {
        ...
        return entries.map(function (entry) { return (__assign(__assign({}, entry), { range: __assign({ document: request.document }, intersectingAttr.location.name) })); });
    }
...
var LitAnalyzer = /** @class */ (function () {
    ...
    LitAnalyzer.prototype.getRenameLocationsAtPosition = function (file, position) {
        ...
        if ("document" in renameInfo) {
            ...
            var offset = document.virtualDocument.scPositionToOffset(position);
            var request = this.makeRequest({ file: file, document: document });
            if (document instanceof CssDocument) {
                return [];
            }
...
function getBuiltInHtmlCollection() {
    ...
    var EXTENDED_HTML5_VALUE_MAP = html5.HTML5_VALUE_MAP.map(function (VALUE_MAP) {
        switch (VALUE_MAP.name) {
            case "inputautocomplete":
                return __assign(__assign({}, VALUE_MAP), { values: __spread([{ name: "on" }, { name: "off" }], VALUE_MAP.values) });
            default:
                return VALUE_MAP;
        }
    });
    ...
    return __assign(__assign({}, result), { tags: result.tags.map(function (tag) { return (__assign(__assign({}, tag), { builtIn: true, attributes: addMissingAttrTypes(tag.attributes.map(function (attr) { return (__assign(__assign({}, attr), { builtIn: true })); })) })); }), attrs: addMissingAttrTypes(result.attrs.map(function (attr) { return (__assign(__assign({}, attr), { builtIn: true })); })), events: result.events.map(function (event) { return (__assign(__assign({}, event), { builtIn: true })); }) });
}
function addMissingAttrTypes(attrs) {
    return attrs.map(function (attr) {
        if (hasTypeForAttrName(attr.name) || attr.getType().kind === tsSimpleType.SimpleTypeKind.ANY) {
            ...
            return __assign(__assign({}, attr), { getType: lazy(function () { return newType_1; }) });
        }
        return attr;
    });
}
...
function removeUndefinedFromType(type) {
    switch (type.kind) {
        ...
        case tsSimpleType.SimpleTypeKind.UNION:
            return __assign(__assign({}, type), { types: type.types.filter(function (t) { return !tsSimpleType.isAssignableToSimpleTypeKind(t, tsSimpleType.SimpleTypeKind.UNDEFINED); }) });
    }
    return type;
}
...
function isAssignableToType(_a, request, options) {
    ...
    return tsSimpleType.isAssignableToType(typeA, typeB, request.program, __assign(__assign({}, (inJsFile ? { strict: false } : {})), (options || {})));
}
 ...
 */
function isAssignableBindingUnderSecuritySystem(htmlAttr, _a, request, source) {
    var typeA = _a.typeA, typeB = _a.typeB;
    var securityPolicy = request.config.securitySystem;
...
function checkClosureSecurityAssignability(typeB, htmlAttr, request, source) {
    ...
    if (typeMatch === false) {
        var nominalType = {
            kind: tsSimpleType.SimpleTypeKind.INTERFACE,
            members: [],
        ...
        return [
            {
                ...
                message: "Type '" + tsSimpleType.toTypeString(typeB) + "' is not assignable to '" + overriddenTypes.join(" | ") + "'. This is due to Closure Safe Type enforcement.",
                severity: "error",
                source: source,
...
function isAssignableInAttributeBinding(htmlAttr, _a, request) {
    ...
    if (assignment.kind === HtmlNodeAttrAssignmentKind.BOOLEAN) {
        ...
        if (assignment.kind !== HtmlNodeAttrAssignmentKind.STRING) {
            ...
            // <script src>).
            var securityDiagnostics = isAssignableBindingUnderSecuritySystem(htmlAttr, { typeA: typeA, typeB: typeB }, request, "no-incompatible-type-binding");
...
var rule$1 = {
    ...
    visitHtmlAssignment: function (assignment, request) {
        ...
        if (!tsSimpleType.isAssignableToPrimitiveType(typeB)) {
            if (isAssignableBindingUnderSecuritySystem(htmlAttr, { typeA: typeA, typeB: typeB }, request, this.name) !== undefined) {
...
function convertComponentDeclarationToHtmlTag(declaration, definition, _a) {
    ...
    var _loop_1 = function (event) {
        htmlTag.events.push({
            ...
            name: event.name,
            getType: lazy(function () { return (tsSimpleType.isSimpleType(event.type) ? event.type : tsSimpleType.toSimpleType(event.type, checker)); }),
            fromTagName: tagName,
            builtIn: builtIn
        });
    ...
    var _loop_2 = function (member) {
        ...
        if (member.kind === "property") {
            htmlTag.properties.push(__assign(__assign({}, base), { kind: "property", name: member.propName, required: member.required }));
            if (!("attrName" in member) && addDeclarationPropertiesAsAttributes && (definition != null && !definition.fromLib)) {
                if (declaration.node.getSourceFile().isDeclarationFile) {
                    htmlTag.attributes.push(__assign(__assign({}, base), { kind: "attribute", name: member.propName }));
                }
            }
        }
        if ("attrName" in member && member.attrName != null) {
            htmlTag.attributes.push(__assign(__assign({}, base), { kind: "attribute", name: member.attrName, required: member.required }));
        }
    };
    try {
...
function visitDependencies(node, context) {
    ...
    if (context.ts.isSourceFile(node)) {
        ...
        if (existingResult != null) {
            ...
            var newContext_1 = __assign(__assign({}, context), { 
            ...
            if (this._text == null) {
                ...
                this.parts.forEach(function (part, i) {
                    ...
                    if (typeof part === "string") {
                        ...
                        var substitution = _this.substituteExpression(length, part, prevPart_1, _this.parts[i + 1]);
                        str_1 += substitution;
                    }
                });
                this._text = str_1;
...
function parseHtmlAttrBase(htmlAttrBase) {
    ...
    switch (modifier) {
        case LIT_HTML_EVENT_LISTENER_ATTRIBUTE_MODIFIER:
            return __assign(__assign({ kind: HtmlNodeAttrKind.EVENT_LISTENER }, htmlAttrBase), { modifier: modifier });
        case LIT_HTML_PROP_ATTRIBUTE_MODIFIER:
            return __assign(__assign({ kind: HtmlNodeAttrKind.PROPERTY }, htmlAttrBase), { modifier: modifier });
        case LIT_HTML_BOOLEAN_ATTRIBUTE_MODIFIER:
            return __assign(__assign({ kind: HtmlNodeAttrKind.BOOLEAN_ATTRIBUTE }, htmlAttrBase), { modifier: modifier });
        default:
            return __assign(__assign({ kind: HtmlNodeAttrKind.ATTRIBUTE }, htmlAttrBase), { modifier: undefined });
    }
}
...
function parseHtmlNode(p5Node, parent, context) {
    ...
    htmlNode.attributes = parseHtmlNodeAttrs(p5Node, __assign(__assign({}, context), { htmlNode: htmlNode }));
    return htmlNode;
}
/**
    ...
    switch (typeA.kind) {
        case tsSimpleType.SimpleTypeKind.UNION:
            if (typeB.kind === tsSimpleType.SimpleTypeKind.ANY && typeA.types.find(function (t) { return t.kind === tsSimpleType.SimpleTypeKind.ANY; }) != null) {
                ...
                return __assign(__assign({}, typeA), { types: __spread(typeA.types, [typeB]) });
            }
    }
    return {
        kind: tsSimpleType.SimpleTypeKind.UNION,
...
function mergeRelatedEvents(events) {
    ...
    var _loop_5 = function (event) {
        ...
        if (existingEvent == null) {
            ...
            mergedAttrs.set(name, __assign(__assign({}, existingEvent), { global: existingEvent.global && event.global, description: undefined, getType: lazy(function () { return mergeRelatedTypeToUnion(prevType_2(), event.getType()); }), related: existingEvent.related == null ? [existingEvent, event] : __spread(existingEvent.related, [event]), fromTagName: existingEvent.fromTagName || event.fromTagName }));
        }
    };
    try {
